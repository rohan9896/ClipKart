    ...
    <div className="Wishlist__List">
      {
          state.wishlistArr.map(product => {
              return (
                ...
                <button onClick={() => dispatch({type: "MOVE_TO_CART_FROM_WISHLIST", payload: product.id})} className="primary-button darkBlue">Move To Cart</button>
                <span onClick={() => dispatch({type: "REMOVE_FROM_WISHLIST", payload: product.id})} className="cross">X</span>
              </div>
              )
          })
      }
